...
class _SyncSettingsPageState extends ConsumerState<SyncSettingsPage> {
        ...
        child: InkWell(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(14),
              border: Border.all(
                color: color.withValues(alpha: _busy ? 0.1 : 0.18),
              ),
            ),
            child: Center(
              child: isBusy
  ...
  Future<void> _pushToCloud() async {
    ...
    try {
      ...
      debugPrint(
        'push confirm: uploaded=$uploadedBackup saved=$pushedIsSaved '
        'latest=${await ref.read(syncServiceProvider).checkLatestBackup()}',
      );
      if (pushedIsSaved) {
    ...
    try {
      ...
      final uploadedIsSaved =
          ...
          await ref
              ...
              .verifyBackupMatches(uploadedBackup, zipFile);
      debugPrint(
        'upload confirm: uploaded=$uploadedBackup saved=$uploadedIsSaved '
        'latest=${await ref.read(syncServiceProvider).checkLatestBackup()}',
      );
      if (uploadedIsSaved) {
        await ref.read(syncServiceProvider).markPulledBackup(uploadedBackup);
